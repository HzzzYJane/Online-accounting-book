  ...
  sumbitForm = (event) => {
    ...
    if (price && date && title) {
      if (price < 0) {
        this.setState({
          validatePass: false,
          errorMessage: 'Price cannot be smaller than 0!'
        })     
      } else if (!isValidDate(date)) {
        this.setState({
          validatePass: false,
          errorMessage: 'Please fill in the correct fomat for data!'
        })
      } else {
        this.setState({
          validatePass: true,
      ...
      <form onSubmit={(event) => {this.sumbitForm(event)}} noValidate>
        ...
        <div className="form-group">
          ...
          <input 
            ...
            id="date" placeholder="Enter Data"
            defaultValue={date}
            ref={(input) => {this.dateInput = input}}  
          />
        </div>
        <button type="submit" className="btn btn-primary mr-3">Submit</button>
        <button className="btn btn-secondary" onClick={this.props.onCancelSubmit}> Cancle </button>
